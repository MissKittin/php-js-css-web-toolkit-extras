function kurwoskrypt(element, curse='kurwa')
{
	/*
	 * The Kurwoskrypt
	 * Inserts the word 'kurwa' in random places
	 *
	 * Usage:
	 *  kurwoskrypt(document.body);
	 *  kurwoskrypt(document.body, 'fuq');
	 * or
		document.getElementById('kurwobutton').addEventListener('click', function(){
			kurwoskrypt(document.body);
		}, false);
	 *
	 * Source: http://kurwa.keraj.net/
	 */

	/*
	 * Made by kurwa Kerai
	 *
	 * chcesz rozpowszechniac? podawaj zrodlo
	 * chcesz cos zmieniac? wszystkie komentarze pisz
	 * w komentarzach wieloliniowych, bo sie moze popsuc pod chrome i opera...
	 *
	 * do you want to distribute? provide the source
	 * you want to change something? write all comments
	 * in multi-line comments, because the script may stop working under chrome and opera...
	 */
	...
	function putHere(text)
	{
		...
		/** oj, trzeba to kurwa fixn%C4%85%C4%87 kiedy%C5%9B */
		...
		while(nextAfterWord())
		{
			if(i-- <= 0)
			{
				...
				if(forpe)
				{
					...
					if(lastword == 'si%C4%99')
						continue;

					if(lastword.startsWith('niekt'))
						continue;

					if(lastword.startsWith('r%C3%B3%C5%BCn'))
						continue;
